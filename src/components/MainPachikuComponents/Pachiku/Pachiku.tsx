...
export default function Pachiku({ pachiku }: PachikuProps) {
	...
	const [initialHeartState, setInitialHeartState] = useState<boolean | null>(null); // State to store the initial heart (like) state. make sure it starts with null the logic depends on that.
	...
	// Handler for clicking on the post body to open the post detail
	function handlePostClick(e: React.MouseEvent) {
		...
		router.push(`/pachiku-page/${pachiku.id}`);
	}

	return (
		<PachikuContext.Provider value={pachiku}>
			<li className={styles.pachiku} key={pachiku.id} onClick={handlePostClick}>
				...
				<LikesCommentsShare initialHeartState={initialHeartState} pachiku={pachiku} />
			</li>
