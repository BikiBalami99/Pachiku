...
export function HeartIcon({
    ...
    const [heartClassName, setHeartClassName] = useState(styles.heartIcon);
    ...
    async function handleClick() {
        ...
        const pachikuRes = await fetch(`/api/pachiku?pachikuId=${pachikuId}`);
        const pachikuData = await pachikuRes.json();

        if (likeData.error) {
            console.error(likeData.error);
            // If there's an error, revert the optimistic update
            setHeartClicked((prev) => !prev);
        } else {
            setNumOfLikes(pachikuData.likes);
        }
    }
...
export function CommentIcon() {
    return (
        // key is set to Math.random to debug lagging issues with the state of the heart clicked
        <svg
            key={Math.random()}
            width="22"
            height="22"
        ...
        >
            <path
                id="Ellipse 2"
                d="M1 10.8182C1 16.2406 5.31725 20.6364 10.6429 20.6364C11.7699 20.6364 12.8518 20.4395 13.8571 20.0777C17.0714 19.9091 20.2857 20.6364 21 21C21 20.2727 19.5714 17.5909 18.5 16.5C19.3333 15.6515 20.2857 13.1455 20.2857 10.8182C20.2857 5.39575 15.9685 1 10.6429 1C5.31725 1 1 5.39575 1 10.8182Z"
            />
        </svg>
    );
}
...
export function ShareIcon() {
    return (
        ...
        >
            <path
                id="Vector 1"
                d="M1 13V20.1579C1.11616 20.7719 1.62727 22 2.74242 22C3.85758 22 16.2172 22 22.2576 22C22.8384 21.8772 24 21.3368 24 20.1579C24 18.9789 24 16.193 24 13M12.5 17.2105V1M12.5 1L19.1212 6.89474M12.5 1L5.5303 6.89474"
            />
        </svg>
    );
}
